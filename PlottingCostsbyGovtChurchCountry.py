...
#for index, df in enumerate(table_names):
#    print(f"Index: {index}, DataFrame: {df}")
...
sums_US_govt_costs = [] # Initialize list to store sums
sums_US_church_costs = []
...
for table_df in table_names:
    ...
    sum_US = table_df.loc[table_df.loc[:, 'Country'] == "US", 'Cost to govt'].sum()  # 18th column (zero-based index)
    sums_US_govt_costs.append(sum_US)

    # Calculate sum of the 19th column and append to the list
    sum_US_church = table_df.loc[(table_df.loc[:, 'Country'] == "US") & (table_df.loc[:, 'Denomination'] != "N/A"), 'Cost to govt'].sum()  # 19th column (zero-based index)
    sums_US_church_costs.append(sum_US_church)

       # Calculate sum of the 18th column and append to the list
    sum_BC = table_df.loc[table_df.iloc[:, 1] == "BC", 'Cost to govt'].sum()  # 18th column (zero-based index)
    sums_BC_govt_costs.append(sum_BC)

    # Calculate sum of the 19th column and append to the list
    sum_BC_church = table_df.loc[(table_df.iloc[:, 1] == "BC") & (table_df.loc[:, 'Denomination'] != "N/A"), 'Cost to govt'].sum()  # 19th column (zero-based index)
    sums_BC_church_costs.append(sum_BC_church)
...
plt.plot(years, sums_US_govt_costs, label='Cost to US Government', marker='o')
plt.plot(years, sums_US_church_costs, linestyle='dotted', label='Cost to US Churches', marker='o')
